...
BOOST_AUTO_TEST_CASE(advanced_var)
{
	struct WithMoveConstructorOnly : public CMovable
	{
		WithMoveConstructorOnly() = default;

		WithMoveConstructorOnly(const WithMoveConstructorOnly&) = delete;
		WithMoveConstructorOnly(WithMoveConstructorOnly&& other) : CMovable(std::move(other)) {}

		WithMoveConstructorOnly& operator =(const WithMoveConstructorOnly& other)
		{
			CMovable::operator=(static_cast<const CMovable&>(other));
			return *this;
		}

		WithMoveConstructorOnly& operator =(WithMoveConstructorOnly&& other)
		{
			CMovable::operator=(static_cast<CMovable&&>(other));
			return *this;
		}
	};
	static_assert(!prop_react::detail::copy_constructible<WithMoveConstructorOnly>);
	static_assert(std::move_constructible<WithMoveConstructorOnly>);

	{
		RESET_COUNTERS;
		prop_react::property<WithMoveConstructorOnly> test;

		{
			test.var();
		}

		BOOST_CHECK_MOVING_(1, 0, 1, 1, 0, 1); // when two of copy/move assign are available prefer move assign
	}

	struct WithCopyConstructorOnly : public CMovable
	{
		WithCopyConstructorOnly() = default;

		WithCopyConstructorOnly(const WithCopyConstructorOnly& other) : CMovable(other) {}
		WithCopyConstructorOnly(WithCopyConstructorOnly&& other) = delete;

		WithCopyConstructorOnly& operator =(const WithCopyConstructorOnly& other)
		{
			CMovable::operator=(static_cast<const CMovable&>(other));
			return *this;
		}

		WithCopyConstructorOnly& operator =(WithCopyConstructorOnly&& other)
		{
			CMovable::operator=(static_cast<CMovable&&>(other));
			return *this;
		}

		bool operator == (const WithCopyConstructorOnly&) const { return false; }
	};
	static_assert(prop_react::detail::copy_constructible<WithCopyConstructorOnly>);
	static_assert(!std::move_constructible<WithCopyConstructorOnly>);

	{
		RESET_COUNTERS;
		prop_react::property<WithCopyConstructorOnly> test;

		{
			test.var();
		}

		BOOST_CHECK_MOVING_(1, 1, 0, 1, 0, 1); // when two of copy/move assign are available prefer move assign
	}

	struct WithMoveCopyConstructorAndCopyAssignOnly : public CMovable
	{
		WithMoveCopyConstructorAndCopyAssignOnly() = default;

		WithMoveCopyConstructorAndCopyAssignOnly(const WithMoveCopyConstructorAndCopyAssignOnly& other) : CMovable(other) {}
		WithMoveCopyConstructorAndCopyAssignOnly(WithMoveCopyConstructorAndCopyAssignOnly&& other) : CMovable(std::move(other)) {}

		WithMoveCopyConstructorAndCopyAssignOnly& operator =(const WithMoveCopyConstructorAndCopyAssignOnly& other)
		{
			CMovable::operator=(static_cast<const CMovable&>(other));
			return *this;
		}

		WithMoveCopyConstructorAndCopyAssignOnly& operator =(WithMoveCopyConstructorAndCopyAssignOnly&& other) = delete;

		bool operator == (const WithMoveCopyConstructorAndCopyAssignOnly&) const
		{
			return false;
		}
	};
	static_assert(prop_react::detail::copy_constructible<WithMoveCopyConstructorAndCopyAssignOnly>);
	static_assert(std::move_constructible<WithMoveCopyConstructorAndCopyAssignOnly>);

	{
		RESET_COUNTERS;
		prop_react::property<WithMoveCopyConstructorAndCopyAssignOnly> test;

		{
			test.var();
		}

		BOOST_CHECK_MOVING_(1, 1, 0, 1, 1, 0); // when two of copy/move assign are available prefer move assign
	}
}
